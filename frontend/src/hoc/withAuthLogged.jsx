...
const withAuth = (WrappedComponent) => {
  return (props) => {
    const [isCheckingAuth, setIsCheckingAuth] = useState(true); // Ajout d'un état

    useLayoutEffect(() => {
      const token = localStorage.getItem('token');

      if (token) {
        redirect('/'); // Redirection immédiate si pas de token
      } else {
        setIsCheckingAuth(false); // Authentification terminée
      }
    }, []);

    // Ne rien afficher tant que l'authentification n'est pas terminée
    if (isCheckingAuth) {
      return <div className="bg-white dark:bg-gray-800 flex justify-center items-end w-screen min-h-[calc(100vh - 66px)] p-5">
      <svg
    ...
    // Sinon, on rend le composant protégé
